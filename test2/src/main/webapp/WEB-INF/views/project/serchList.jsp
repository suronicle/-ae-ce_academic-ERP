  ...
  <div class="content-wrapper">
    ...
    <div class="card-body">
     	<table class="table">
     		...
     		<tr>
     		<td><a href="attenDetail?aId=${serchList.aId}&aDate=${serchList.aDate}"> ${serchList.aId}</a></td>
     		<td>${serchList.aDate}</td>
     		<td><c:choose>
     		<c:when test="${serchList.aAttend == 2}">지각</c:when>
     		<c:when test="${serchList.aAttend == 3}">조퇴</c:when>
     		<c:when test="${serchList.aAttend == 4}">연차</c:when>
     		<c:when test="${serchList.aAttend == 5}">반차</c:when>
     		<c:when test="${serchList.aAttend == 6}">결석</c:when>
     		</c:choose></td>
     		<td><c:choose>
     		...
     		<td style="<c:choose><c:when test="${serchList.aConfirm == 101}">color:skyblue;</c:when> <c:when test="${serchList.aConfirm == 102}">color:red;</c:when></c:choose>">
     		<c:choose><c:when test="${serchList.aConfirm == 100}">미승인</c:when>
     		<c:when test="${serchList.aConfirm == 101}">승인</c:when>
     		<c:when test="${serchList.aConfirm == 102}">반려</c:when>
     		</c:choose></td>
     		<td>${serchList.aDeny}</td>
     		</tr>
     		</c:forEach>
...
<script>
		document.getElementById("searchBtn").onclick = function () {
			let searchType = document.getElementsByName("serachField")[0].value;
			let keyword = document.getElementsByName("serachText")[0].value;
